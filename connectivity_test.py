...
def remote_url_service_function(src_ip_addr):
    x = subprocess.check_output("cat configuration_and_log/configuration_url_service.txt | grep -v '#' | sed '/^$/d;s/[[:blank:]]//g' > configuration_and_log/configuration.txt", shell=True)
    ...
    for line in Lines:
        ...
        if m1:
            ...
            if m2:
                ...
                port = m4.group().strip(':')
                print("Port ==> "+ port +"")
                y = subprocess.call("ssh normaluser@"+ src_ip_addr +" nc -vz -w 5 "+ host +" "+ port +"", shell=True)
...
def remote_manual_change_function(src_ip_addr):
    x = subprocess.check_output("cat configuration_and_log/configuration_manual_change.txt | grep -v '#' | sed '/^$/d;s/[[:blank:]]//g' > configuration_and_log/configuration1.txt", shell=True)
...
def remote_manual_input(src_ip_addr):
    n = int(input("Enter the number of servers you have for test\n"))
    print("*****************Checking the connection starts****************")
...
if __name__=="__main__":
    print('''Press 1 if you want to start the connectivity test from this server
press 2 if you want to start the connectivity test from another server''')
...
Press 3 for giving input of host and port from keyboard everytime''')
        ...
        src_ip_addr = str(input("Enter the source IP address or FQDN\n"))
        print('''Press 1 for the URL service type connectivity test
Press 2 for Manual configuration type connectivity test
Press 3 from giving input of host and port from keyboard everytime''')
        option = int(input())
        if( option==1):
            print("***************Checking the connection starts**************")
            remote_url_service_function(src_ip_addr)
        elif ( option==2):
            print("***************Checking the connection starts**************")
            remote_manual_change_function(src_ip_addr)
        elif( option==3):
            remote_manual_input(src_ip_addr)
